...
begin
	...
	--Remove semicolons from these:
		...
		'ALTER DATABASE LINK','ALTER DIMENSION','ALTER DISK GROUP','ALTER DOMAIN', 'ALTER EDITION','ALTER FLASHBACK ARCHIVE',
		'ALTER FUNCTION','ALTER INDEX','ALTER INDEXTYPE','ALTER JAVA','ALTER LIBRARY','ALTER MATERIALIZED VIEW ',
		'ALTER MATERIALIZED VIEW LOG','ALTER MATERIALIZED ZONEMAP','ALTER MLE ENV','ALTER MLE MODULE','ALTER OPERATOR','ALTER OUTLINE',
		'ALTER PACKAGE','ALTER PACKAGE BODY','ALTER PLUGGABLE DATABASE','ALTER PMEM FILESTORE','ALTER PROCEDURE','ALTER PROFILE',
		'ALTER PROPERTY GRAPH','ALTER RESOURCE COST',/*'ALTER REWRITE EQUIVALENCE',*/'ALTER ROLE','ALTER ROLLBACK SEGMENT',
		...
		'CREATE DATABASE LINK','CREATE DIMENSION','CREATE DIRECTORY','CREATE DISK GROUP','CREATE DOMAIN', 'CREATE EDITION',
		...
		'CREATE MLE ENV',/*'CREATE MLE MODULE',*/'CREATE OPERATOR','CREATE OUTLINE',/*'CREATE PACKAGE',*//*'CREATE PACKAGE BODY',*/'CREATE PFILE',
		'CREATE PLUGGABLE DATABASE','CREATE PMEM FILESTORE',/*'CREATE PROCEDURE',*/'CREATE PROFILE','CREATE PROPERTY GRAPH','CREATE RESTORE POINT','CREATE ROLE',
		...
		'DROP DOMAIN', 'DROP EDITION','DROP FLASHBACK ARCHIVE','DROP FUNCTION','DROP INDEX','DROP INDEXTYPE','DROP JAVA',
		'DROP LIBRARY','DROP MATERIALIZED VIEW ','DROP MATERIALIZED VIEW  LOG','DROP MATERIALIZED ZONEMAP',
		'DROP MLE ENV','DROP MLE MODULE','DROP OPERATOR','DROP OUTLINE','DROP PACKAGE','DROP PACKAGE BODY','DROP PLUGGABLE DATABASE',
		'DROP PMEM FILESTORE','DROP PROCEDURE','DROP PROFILE','DROP PROPERTY GRAPH','DROP RESTORE POINT',/*'DROP REWRITE EQUIVALENCE',*/'DROP ROLE',
	...
	--Do nothing for these, they can end with a semicolon:
		...
		'CREATE JAVA',
		'CREATE LIBRARY',
		-- MLE Module is unusual - it works both with and without a comma in SQL.
		-- However, since SQL*Plus needs the slash, it seems safer to include the last semicolon.
		-- For example, the same command runs in the below PL/SQL block both with and without the semicolon.
		--begin
		--	execute immediate 'create or replace mle module test_module language JAVASCRIPT as ''asdf asdf''';
		--	execute immediate 'create or replace mle module test_module language JAVASCRIPT as ''asdf asdf'';';
		--end;
		'CREATE MLE MODULE',
		'CREATE PACKAGE',
