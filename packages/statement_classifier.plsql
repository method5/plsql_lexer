...
--Copyright (C) 2023 Jon Heller.  This program is licensed under the LGPLv3.
...
- p_command_type (OUT):  This is an ID number assigned to each command name,
	...
	Does not include these values that are in V$SQLCOMMAND but are either not
	in the manual, SQL*Plus comands, or not used:
		285 DESCRIBE
		278 CREATE TRUE CACHE
		273 ANALYZE STORAGEPOOL
		272 LOAD
		271 DROP LANGUAGE
		270 CREATE LANGUAGE
		265 DROP LOGICAL PARTITION TRACKING
		264 ALTER LOGICAL PARTITION TRACKING
		263 CREATE LOGICAL PARTITION TRACKING
		262 DROP BLOCKCHAIN (This is not a regular blockchain table.)
		261 ALTER BLOCKCHAIN (This is not a regular blockchain table.)
		260 CREATE BLOCKCHAIN (This is not a regular blockchain table.)
...
begin
	...
	CREATE [UNIQUE|BITMAP|MULTIVALUE|SEARCH]? INDEX
	...
	CREATE [BIGFILE|SMALLFILE]? [TEMPORARY|UNDO|LOCAL TEMPORARY]? TABLESPACE
	...
	CREATE [OR REPLACE]? [NOFORCE|FORCE]? [EDITIONING|EDITIONABLE|EDITIONABLE EDITIONING|NONEDITIONABLE][JSON DUALITY|JSON RELATIONAL DUALITY]? VIEW
	DROP [PUBLIC]? DATABASE LINK
	DROP [PUBLIC]? SYNONYM
	SET [CONSTRAINT|CONSTRAINTS]* (AMBIGUOUS - HOW DO WE NAME THESE?)

	TODO: I should rewrite this optional keyword code. It's not 100% accurate. For example, "CREATE OR REPLACE TABLE" should not be allowed.
	*/
	for i in p_start_index .. p_tokens.count loop
		if p_tokens(i).type not in ('whitespace', 'comment')
			...
			(
				...
				(
					--Removed COMPILE
					'AND','BIGFILE','BITMAP','EDITIONABLE','EDITIONING','FORCE','FORCE',
					'MULTIVALUE','NO','NOFORCE','NONEDITIONABLE','OR','PUBLIC','REPLACE','RESOLVE',
					'SHARED','SMALLFILE','UNDO','UNIQUE'
				)
			)
		then
	...
	--Get up to 8 tokens.
	--The first 5 tokens are sufficient 99% of the time.
	...
	for i in 1 .. least(v_concrete_tokens.count, 8) loop
		if v_concrete_tokens.exists(i) then
			...
			elsif v_types(i) = 'word' and i = 5 then
				v_words_1_to_5 := upper(v_words_1_to_4 || ' ' || v_concrete_tokens(i).value);
			end if;
		end if;
	end loop;
	...
	*/
	...
	elsif v_words_1_to_2 = 'ALTER DOMAIN' then
		p_category := C_DDL; p_statement_type := 'ALTER'; p_command_name := 'ALTER DOMAIN'; p_command_type := 282;
	...
	elsif v_words_1_to_3 = 'ALTER MLE ENV' then
		p_category := C_DDL; p_statement_type := 'ALTER'; p_command_name := 'ALTER MLE ENV'; p_command_type := 280;
	elsif v_words_1_to_3 = 'ALTER MLE MODULE' then
		p_category := C_DDL; p_statement_type := 'ALTER'; p_command_name := 'ALTER MLE MODULE'; p_command_type := 279;
	...
	elsif v_words_1_to_3 = 'ALTER PMEM FILESTORE' then
		p_category := C_DDL; p_statement_type := 'ALTER'; p_command_name := 'ALTER PMEM FILESTORE'; p_command_type := 258;
	...
	elsif v_words_1_to_3 = 'ALTER PROPERTY GRAPH' then
		p_category := C_DDL; p_statement_type := 'ALTER'; p_command_name := 'ALTER PROPERTY GRAPH'; p_command_type := 276;
...
--ALTER LOCAL TABLESPACE
	...
	elsif v_words_1_to_2 = 'CREATE DOMAIN' or v_words_1_to_3 = 'CREATE FLEXIBLE DOMAIN' then
		p_category := C_DDL; p_statement_type := 'CREATE'; p_command_name := 'CREATE DOMAIN'; p_command_type := 281;
	...
	elsif v_words_1_to_3 = 'CREATE MLE ENV' then
		p_category := C_DDL; p_statement_type := 'CREATE'; p_command_name := 'CREATE MLE ENV'; p_command_type := 268;
	elsif v_words_1_to_3 = 'CREATE MLE MODULE' then
		p_category := C_DDL; p_statement_type := 'CREATE'; p_command_name := 'CREATE MLE MODULE'; p_command_type := 266;
	...
	elsif v_words_1_to_2 = 'CREATE OUTLINE' or v_words_1_to_3 in ('CREATE PRIVATE OUTLINE', 'CREATE PUBLIC OUTLINE') then
	...
	elsif v_words_1_to_3 = 'CREATE PMEM FILESTORE' then
		p_category := C_DDL; p_statement_type := 'CREATE'; p_command_name := 'CREATE PMEM FILESTORE'; p_command_type := 257;
	...
	elsif v_words_1_to_2 = 'CREATE PROFILE' or v_words_1_to_3 = 'CREATE MANDATORY PROFILE' then
		p_category := C_DDL; p_statement_type := 'CREATE'; p_command_name := 'CREATE PROFILE'; p_command_type := 65;
	elsif v_words_1_to_3 = 'CREATE PROPERTY GRAPH' then
		p_category := C_DDL; p_statement_type := 'CREATE'; p_command_name := 'CREATE PROPERTY GRAPH'; p_command_type := 275;
	elsif v_words_1_to_4 = 'CREATE CLEAN RESTORE POINT' or v_words_1_to_3 = 'CREATE RESTORE POINT' then
	...
	--	p_category := C_DDL; p_statement_type := 'CREATE'; p_command_name := 'CREATE SUMMARY'; p_command_type := 171;
		...
		(
			v_words_1_to_2 = 'CREATE TABLE'
			or v_words_1_to_3 in ('CREATE SHARDED TABLE', 'CREATE DUPLICATED TABLE', 'CREATE IMMUTABLE TABLE', 'CREATE BLOCKCHAIN TABLE')
			or v_words_1_to_4 in ('CREATE GLOBAL TEMPORARY TABLE', 'CREATE PRIVATE TEMPORARY TABLE', 'CREATE IMMUTABLE BLOCKCHAIN TABLE')
		) then
		...
		(
			v_words_1_to_2 = 'CREATE TABLESPACE'
			or v_words_1_to_3 in ('CREATE TEMPORARY TABLESPACE', 'CREATE LOCAL TABLESPACE')
			or v_words_1_to_4 = 'CREATE LOCAL TEMPORARY TABLESPACE'
		) then
	...
	--Although listed in a separate page of SQL Language Reference, JSON duality views are still just regular views.

	elsif v_words_1_to_2 = 'CREATE VIEW' or v_words_1_to_4 = 'CREATE JSON DUALITY VIEW' or v_words_1_to_5 = 'CREATE JSON RELATIONAL DUALITY VIEW' then
	...
	*/
	...
	elsif v_words_1_to_2 = 'DROP DOMAIN' then
		p_category := C_DDL; p_statement_type := 'DROP'; p_command_name := 'DROP DOMAIN'; p_command_type := 283;
	...
	elsif v_words_1_to_3 = 'DROP MLE ENV' then
		p_category := C_DDL; p_statement_type := 'DROP'; p_command_name := 'DROP MLE ENV'; p_command_type := 269;
	elsif v_words_1_to_3 = 'DROP MLE MODULE' then
		p_category := C_DDL; p_statement_type := 'DROP'; p_command_name := 'DROP MLE MODULE'; p_command_type := 267;
	...
	elsif v_words_1_to_3 = 'DROP PMEM FILESTORE' then
		p_category := C_DDL; p_statement_type := 'DROP'; p_command_name := 'DROP PMEM FILESTORE'; p_command_type := 259;
	...
	elsif v_words_1_to_3 = 'DROP PROPERTY GRAPH' then
		p_category := C_DDL; p_statement_type := 'DROP'; p_command_name := 'DROP PROPERTY GRAPH'; p_command_type := 277;
	...
	*/
	...
	--only be run as SYSASM and will probably never be used in real life.
		...
		(
			v_words_1_to_2 = 'FLASHBACK DATABASE'
			or v_words_1_to_3 in ('FLASHBACK STANDBY DATABASE', 'FLASHBACK PLUGGABLE DATABASE')
			or v_words_1_to_4 in ('FLASHBACK STANDBY PLUGGABLE DATABASE')
		) then
