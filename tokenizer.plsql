...
begin
	...
	if g_last_char_position > g_chars.count or g_chars.count = 0 then
		return token('EOF', g_last_char_position, null, null, null);
	end if;
	...
	if is_lexical_whitespace(g_last_char) then
		...
		return token('whitespace', g_last_char_position, g_token_text, null, null);
	end if;
	...
	if g_last_char = '-' and look_ahead(1) = '-' then
		...
		return token('comment', g_last_char_position, g_token_text, null, null);
	end if;
	...
	if g_last_char = '/' and look_ahead(1) = '*' then
		...
		loop
			...
			if look_ahead(1) is null then
				g_token_text := g_token_text || g_last_char;
				g_last_char := get_char;
				return token('comment', g_last_char_position, g_token_text, -01742, 'comment not terminated properly');
			end if;
			g_token_text := g_token_text || g_last_char;
		end loop;
		return token('comment', g_last_char_position, g_token_text, null, null);
	...
	if g_last_char = '''' then
		...
		loop
			...
			if g_last_char = '''' and look_ahead(1) = '''' then
				...
				g_token_text := g_token_text || g_last_char;
				g_last_char := get_char;
				return token('text', g_last_char_position, g_token_text, -01756, 'quoted string not properly terminated');
			end if;
			g_token_text := g_token_text || g_last_char;
		end loop;
		return token('text', g_last_char_position, g_token_text, null, null);
	end if;
		...
		loop
			...
			if g_last_char = v_quote_delimiter and look_ahead(1) = '''' then
				...
				if g_last_char in (chr(9), chr(10), chr(32)) then
					...
					g_token_text := g_token_text || g_last_char;
					g_last_char := get_char;
					return token('text', g_last_char_position, g_token_text, -00911, 'invalid character');
				end if;
	...
	if g_last_char between '0' and '9' or (g_last_char = '.' and look_ahead(1) between '0' and '9') then
		...
		g_last_char := get_char;
		return token('numeric', g_last_char_position, g_token_text, null, null);
	end if;
	...
	if g_last_char = '"' then
		...
		loop
			...
			if look_ahead(1) is null then
				g_token_text := g_token_text || g_last_char;
				g_last_char := get_char;
				return token('word', g_last_char_position, g_token_text, -01740, 'missing double quote in identifier');
			end if;
			g_token_text := g_token_text || g_last_char;
		end loop;
		return token('word', g_last_char_position, g_token_text, null, null);
	end if;
	...
	if g_last_char = '$' and look_ahead(1) = '$' and is_alpha(look_ahead(2)) then
		...
		return token('inquiry_directive', g_last_char_position, g_token_text, null, null);
	end if;
	...
	if g_last_char = '$' and is_alpha(look_ahead(1)) then
		...
		return token('preprocessor_control_token', g_last_char_position, g_token_text, null, null);
	end if;
	...
	if g_last_char||look_ahead(1)||look_ahead(2) in (',}?') then
		...
		g_last_char := get_char;
		return token(g_token_text, g_last_char_position, g_token_text, null, null);
	end if;
	...
	g_token_text := g_last_char;
	g_last_char := get_char;
	return token('unexpected', g_last_char_position, g_token_text, null, null);
end get_token;
...
begin
	...
	g_last_concrete_token := token(null, null, g_last_char_position, null, null);
